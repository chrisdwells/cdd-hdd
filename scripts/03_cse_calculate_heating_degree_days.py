...
Here we just separate out into Heating and Cooling degree days - Heating in this script.

We set the heating threshold at 18C
...
output_var = 'heating_degree_days'
...
#%% Calculate heating degree days temperature
#------------------------------------------------------------------------------
...
for GCM, RCP in it.product(GCMs, RCPs):
    ...
    if make_ref:
        ...
        if len([val for key,val in years.items() if f'{GCM}_{RCP}' in key and len(val) != 0]) > 0:
            ...
            heating_degree_days_ref = cfi.calculate_heating_degree_days(tas_ref, 
                                                   balance_temperature_heating_low)
            
            heating_degree_days_ref_mean = heating_degree_days_ref.mean(dim='year') 
            
            # Save averaged data
            output_file = cf.create_output_file(output_dir, GCM, RCP, 'historical', 
                                                reference_period, 
                                                f'{output_var}{input_var.replace("tas", "")}_{str(balance_temperature_heating_low).replace(".", "p")}')
            cf.write_output(heating_degree_days_ref_mean, output_file, output_var)
    ...
    for gwl in GWLs:
        ...
        if not years[combination]:
            ...
            # Calculate indicator for all 31 years and average over all years
            heating_degree_days = cfi.calculate_heating_degree_days(tas, 
                                                    balance_temperature_heating_low)
            heating_degree_days_mean = heating_degree_days.mean(dim='year') 
            
            # Save averaged data
            output_file = cf.create_output_file(output_dir, GCM, RCP, gwl, 
                                                years[combination], 
                                                f'{output_var}{input_var.replace("tas", "")}_{str(balance_temperature_heating_low).replace(".", "p")}')
            cf.write_output(heating_degree_days_mean, output_file, output_var)  
