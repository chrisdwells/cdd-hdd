...
# This script generates population-mean CDD and HDD, for the globe and some 
# large regions. It then fits different curves to the aggregated data, 
# as a function of GMST.
...
regional = True
region_datadir = '../required_files'
cube_r10 = iris.load_cube(f'{region_datadir}/r10masks_fractional.nc')
    ...
    for gcm in GCMs:
        ...
        for gwl in gwls:
            ...
            for var_i, var in enumerate(varlist):
                ...
                cb = plt.colorbar(
                    mesh_1, extend="neither", orientation="horizontal", shrink=0.6, 
                    pad=0.05, label='People*Degree*Days/yr'
                    )
...
# Skip the above steps if already ran
