# -*- coding: utf-8 -*-
"""
Directly taken from https://github.com/iiasa/cse_impact_data/blob/main/cse_functions_indicators.py
...
def calculate_cooling_degree_days(TAS, balance_temperature_cooling_low):
    ...
    cooling_degree_days = calc_dd_c(TAS.tas, balance_temperature_cooling_low)        
    cooling_degree_days = xr.Dataset(data_vars={'cooling_degree_days':cooling_degree_days})
    
    cooling_degree_days_yearly = cooling_degree_days.groupby('time.year').sum(dim='time', skipna=True)
    
    return cooling_degree_days_yearly  

# -----------------------------------------------------------------------------

def calculate_heating_degree_days(TAS, balance_temperature_heating_low):
    
    ''' Calculate heating degree days 
    
        Arguments:
            - TAS: xarray dataset with global daily temperature
            - balance_temperature_heating_low: balance temperature for heating degree days in Celsius '''
    
    heating_degree_days = calc_dd_h(TAS.tas, balance_temperature_heating_low)        
    heating_degree_days = xr.Dataset(data_vars={'heating_degree_days':heating_degree_days})
    
    heating_degree_days_yearly = heating_degree_days.groupby('time.year').sum(dim='time', skipna=True)
    
    return heating_degree_days_yearly  
