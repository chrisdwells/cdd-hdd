...
Edited file location only - for 3b.
...
def set_protocol(protocol):
    ...
    if protocol == '2b':
        ...
        input_dir = "/nfs/annie/earcwel/isimip-climate/from_archive"
...
def create_file_list(input_dir, GCM, RCP, var, timestep, GHM=''):
    ...
    file_path_future = f'{os.path.join(input_dir)}/{GCM.lower()}*{RCP}_{var}*.nc*'
    file_list_future = glob.glob(file_path_future)
       
    # Create file path and list for historical years
    file_path_hist = f'{os.path.join(input_dir)}/{GCM.lower()}*historical_{var}*.nc*'
...
def create_output_file(output_dir, GCM, RCP, threshold, years, var, GHM=''):
    ...
    if not GHM:
        ...
        data_file = os.path.join(output_dir, output_formatted)
